...
large_p = 4784032922807635434641496274794020734215792346746034989082343043839897113276391971239984361749303692596852162547046369143100363824064261924178278203171859767009395429173719172951486340792843731140609 #Large Modulus for encryption
max_p = 17592182833153                   # large_p = Product of p_i, max_p = max(p_i)
min_p = 17592173002753                   # min_p = min(p_i)
n_p = 15                                 # number of p_i
ring_dim = 32768                         # Encryption ring dimension
Berr = 19.2                              # Error InfinityNorm*TrustCoeficient
rho2 = 167                               # vector A size = rho+2, derived from trapdoor generation
tp = 65537                                # Plaintext modulus
...
logq1 = 63                              # log of the smallest prime divisor of q, we want to have a prime close to 3329
logq = 63                               # log of the proof system modulus q
...
k = ring_dim / d                         # Number of polynomials required in the commitment to represent a encryption polynomial
if( floor(k)*d != ring_dim ):             # if d|ring_dim the solution from [] does not apply 
    print("ERROR: commitment ring must divide encryption ring")
beta = floor(math.log2(tp))               # Decomposition of one plaintext mod t into beta binary polynomials 
M = 32                                   # Number of bits in the pseudo-random generator. key space = 2**M

m1 = k*(beta+rho2+3)+2                   # length of s1
m2 = 0                                   # length of s2, to be determined
ell = 2*k+M+(secpam/(2*logq1))+1         # length of m
alpha_nb = Berr*sqrt(2*k*d)              # norm non binary subvector in s1
alpha_b = sqrt(d*(beta*k+M/2+k*rho2))  # norm of binary subvector in s1
alpha = sqrt(alpha_nb^2+alpha_b^2)
...
ve=1
BoundsToProve = [ Berr*sqrt(k*d) ] 
k_bin = k*beta+ceil(k/2)+k*rho2+2
alphae = sqrt(Berr^2*k*d + (k_bin + ve)*d)# bound alpha^(e) on the vector e^(e) = (r,m, bin. decomp. of B^2 - ||r||^2)
ce = k + k_bin + ve                             # length of the vector e^(e)
approximate_norm_proof = 2 
alphad = (sqrt(d*rho2)/2+alpha_s1/min_p+1)*sqrt(2*d*n_p*k)
...
stdev1_nb = gamma1 * eta * alpha_nb
stdev1_b = gamma1 * eta * sqrt(alpha_b^2 + ve * d)
stdev2 = 0
...
mlwe =  20                               # dimension of the Module-LWE problem
mlwe_hardness = 2
...
if q <= min_p*(sqrt(d*rho2)/2+alpha_s1/max_p+1+Bd):
...
print("Log2 of the standard deviation stdev1_nb: ",round(log(stdev1_nb,2),2))
print("Log2 of the standard deviation stdev1_b: ",round(log(stdev1_b,2),2))
...
short_size1 = (m1 -k*rho2) * d * (ceil(log(stdev1_nb,2) + 2.57)) + (m2 - n) * d * (ceil(log(stdev2,2) + 2.57))
short_size2 = 256 * (ceil(log(stdeve,2) + 2.57)) + approximate_norm_proof * 256 * (ceil(log(stdevd,2) + 2.57))
short_size3 = (k * rho2 +ve)* d * (ceil(log(stdev1_b,2) + 2.57)) 

hint = 2.25 * n * d

print("Total proof size in KB: ", round((full_size + challenge + short_size1 + short_size2 + +short_size3 + hint)/(2^13) , 2))
print("full-sized polynomials in KB: ", round(full_size/(2^13) , 2))
print("challenge c in KB: ", round(challenge/(2^13) , 2))
print("short-sized polynomials in KB: ", round((short_size1 + short_size2 + short_size3 + hint)/(2^13) , 2))
